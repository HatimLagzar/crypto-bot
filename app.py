...
class USDTDominanceAnalyzer:
    ...
    def get_usdt_dominance_data(self) -> Optional[pd.DataFrame]:
        """Fetch USDT dominance history directly from TradingView.

        Uses tvDatafeed to pull USDT.D (CRYPTOCAP:USDT.D) which represents
        ...
        try:
            ...
            if df is None or len(df) < 10:
                logger.warning("Insufficient USDT.D data from TradingView")
                return None

            # tvDatafeed returns: open, high, low, close, volume
            # Use 'close' as the dominance value
            df = df[['close']].copy()
            df.rename(columns={'close': 'dominance'}, inplace=True)

            current_dom = df['dominance'].iloc[-1]
            logger.info(
                f"Fetched {len(df)} USDT.D data points from TradingView "
                f"(current: {current_dom:.3f}%)"
            )
            return df
